...
// THE VISITOR HAS CLICKED THE PAYPAL CHECKOUT BUTTON
	...
	*/
	...
	// SEND CART CONTENTS TO PAYPAL USING THEIR UPLOAD METHOD, FOR DETAILS SEE http://tinyurl.com/djoyoa
		...
		// création commande
		$cmd_id = $cart->create_order($_REQUEST['payment_method']);
		
		// paiement en ligne
		if ($_REQUEST['payment_method'] == 'CB') {
		...
		foreach ($cart->get_contents() as $item)
			...
			$items_query_string .= '&tax_rate_' . $paypal_count . '=' . $item['tax'] * 100;
		...
		if($jcart['paypal_id'])
			...
			header('Location: https://www.paypal.com/cgi-bin/webscr?cmd=_cart&upload=1&business=' . $jcart['paypal_id'] . $items_query_string.'&charset=utf-8&currency_code=EUR&cancel_return='.$jcart['paypal_cancel'].'&notify_url='.$jcart['paypal_notify'].'&return='.$jcart['paypal_return'].'&no_note=0&custom='.$_REQUEST['dedicated']);
			exit;
			}
		else
		  ...
		  // traitements commande ok : mails, exports
  		require_once($context['path_to_root'].'altacart/valid_order.inc.php');
  		valid_order($cmd_id);

		  // merci pour votre commande
      $thanks = Articles::get('thanks');
      $thanks_url = Articles::get_permalink($thanks);
      Safe::redirect($context['url_to_root'].$thanks_url);
    }
